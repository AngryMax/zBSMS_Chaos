...
static inline int __kuribo_fillin_metadata_v0(
    __kuribo_simple_meta_v0 *data, /* Data to fill in. Must be a valid pointer. */
...
#define KURIBO_MODULE_BEGIN(name, author, version)                                                 \
    ...
    int _start(int __reason, __kuribo_module_ctx_t *__ctx) {                                       \
        ...
        case KURIBO_REASON_INQUIRE_META_DESC:                                                      \
            return __kuribo_fillin_metadata_v0(__ctx->udata.fillin_meta, name, author, version);   \
...
#define icbi
#endif  // _WIN32
static inline void __kuribo_multipatch_write(u32 *addr, u32 value, u32 *save, int attach) {
    if (attach) {
        if (save) {
            *save = *addr;
        }
...
static inline void __kuribo_multipatch_b(u32 *addr, u32 value, u32 *save, int attach, int lk) {
    const u32 delta = (u32)value - (u32)addr;
    __kuribo_multipatch_write(addr, 0x48000000 | (delta & 0x3fffffc) | (lk ? 1 : 0), save, attach);
}
...
#define __KURIBO_PATCH_B_IMPL(addr, value, gen, lk)                                                \
    {                                                                                              \
        static u32 gen;                                                                            \
        __kuribo_multipatch_b(((u32 *)addr), ((u32)value), &gen, __kuribo_attach, !!(lk));         \
    }
#undef KURIBO_PATCH_B
#undef KURIBO_PATCH_BL

#define KURIBO_PATCH_B(addr, value)                                                                \
    __KURIBO_PATCH_B_IMPL((addr), (value), MACRO_CONCAT(__kuribo_multisave, __COUNTER__), 0)
#define KURIBO_PATCH_BL(addr, value)                                                               \
    __KURIBO_PATCH_B_IMPL((addr), (value), MACRO_CONCAT(__kuribo_multisave, __COUNTER__), 1)

#define __KURIBO_PATCH_32_IMPL(addr, value, gen)                                                   \
    {                                                                                              \
        static u32 gen;                                                                            \
        __kuribo_multipatch_write(((u32 *)addr), ((u32)value), &gen, __kuribo_attach);             \
    }
#undef KURIBO_PATCH_32
#define KURIBO_PATCH_32(addr, value)                                                               \
    __KURIBO_PATCH_32_IMPL((addr), (value), MACRO_CONCAT(__kuribo_multisave, __COUNTER__))
...
#define KURIBO_GET_PROCEDURE(function) ((void *)__kuribo_ctx.get_procedure(function))
